...
Para practicar lo aprendido, intenta optimizar estas consultas:

```sql
-- EJERCICIO 1: Clientes con compras superiores al promedio
-- (Identifica cómo optimizar esta consulta)
...
WHERE v.TotalVenta > (
    SELECT AVG(TotalVenta) FROM Ventas
)
ORDER BY v.TotalVenta DESC;

-- EJERCICIO 2: Vendedor con mayor comisión total
-- (Piensa en diferentes formas de optimizarlo)

SELECT 
    ve.NombreVendedor,
    SUM(v.TotalVenta * ve.ComisionPorVenta / 100) AS ComisionTotal
FROM Vendedores ve
INNER JOIN Ventas v ON ve.VendedorID = v.VendedorID
GROUP BY ve.NombreVendedor
ORDER BY ComisionTotal DESC;

-- EJERCICIO 3: Productos que nunca se han vendido
-- (Hay múltiples formas de resolverlo)

SELECT 
    p.NombreProducto,
    p.Precio
FROM Productos p
LEFT JOIN DetallesVenta dv ON p.ProductoID = dv.ProductoID
WHERE dv.ProductoID IS NULL;
```

**Pistas para optimizar:**
- El Ejercicio 1 puede usar una variable para el promedio
- El Ejercicio 2 puede usar funciones de ventana para el ranking
- El Ejercicio 3 ya está bastante optimizado con LEFT JOIN
